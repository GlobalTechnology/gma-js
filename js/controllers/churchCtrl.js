	...
	var church_controller = function ( $scope, church_service, ministry_service ) {
		...
		$scope.gridOptions = {
			...
			columnDefs:         [{
									 field:                'development',
									 displayName:          'Type',
									 cellTemplate:         ' <img src="{{getIcon(row.entity[col.field])}}" />',
									 editableCellTemplate: ' <select ng-class="\'colt\' + col.index"  ng-input="COL_FIELD"  ng-model="COL_FIELD" ><option value="1">Target</option><option value="2">Group</option><option value="3">Church</option><option value="5">Multiplying Church</option> </select>'
								 },
								 ...
								 {field: 'contact_name', displayName: 'Contact'},
								 {
									 field:        'contact_email',
									 displayName:  'Email',
									 cellTemplate: ' <a href="mailto:{{row.entity[col.field]}}">{{row.entity[col.field]}}</a>'
								 },
