define( ['gcmApp', 'token', 'ministry_service', 'assignment_service', 'church_service', 'training_service'], function ( gcmApp ) {
	(function ( $ ) {
		gcmApp.controller( 'gcmController', [
			'$scope', '$http', '$timeout', '$filter', 'token', 'ministry_service', 'assignment_service', 'church_service', 'training_service', '$location',
			function ( $scope, $http, $timeout, $filter, token, ministry_service, assignment_service, church_service, training_service, $location ) {
				...
				var onGetSession = function ( data ) {
					...
					if ( $scope.assignments ) {
						$scope.loadAssignment( $scope.assignments[0] );
					} else {
